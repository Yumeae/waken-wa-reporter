...
// GetNowPlaying uses PowerShell and System.Runtime.WindowsRuntime to read GSMTC metadata.
// Higher overhead than the default WinRT build; use tag -tags waken_powershell_media when needed.
...
if (-not $info) { exit 0 }
@{ title = $info.Title; artist = $info.Artist; album = $info.AlbumTitle } | ConvertTo-Json -Compress
`
	...
	info := Info{Title: j.Title, Artist: j.Artist, Album: j.Album}
	if info.IsEmpty() {
		return Info{}, ErrNoMedia
	}
	return info, nil
